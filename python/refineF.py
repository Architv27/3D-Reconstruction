...
def refineF(F, pts1, pts2):
    ...
    initF = F.flatten()  # Flatten F to a one-dimensional array

    # Do the minimization
    res = minimize(svd_distance, initF, args=(X, Y), method='L-BFGS-B', options={'maxiter': 100000, 'maxfun': 10000})
    minF = res.x.reshape((3, 3))  # Reshape the result back to a 3x3 matrix
    F = rank2F(minF)
    return F

def svd_distance(F, X, Y):
    F = F.reshape((3, 3))  # Reshape F to a 3x3 matrix
    F = rank2F(F)
